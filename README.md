# model-load-optimizer

OpenClaw plugin for intelligent Ollama model selection based on GPU/RAM load and model availability

**Built with:** Node.js/JavaScript

## Getting Started

Clone the repository:

```bash
git clone https://github.com/suhteevah/model-load-optimizer.git
cd model-load-optimizer
```

## Installation

```bash
npm install
npm start
```

## License

This project is open source and available under the [MIT License](LICENSE).

---

---

## Support This Project

If you find this project useful, consider buying me a coffee! Your support helps me keep building and sharing open-source tools.

[![Donate via PayPal](https://img.shields.io/badge/Donate-PayPal-blue.svg?logo=paypal)](https://www.paypal.me/baal_hosting)

**PayPal:** [baal_hosting@live.com](https://paypal.me/baal_hosting)

Every donation, no matter how small, is greatly appreciated and motivates continued development. Thank you!
